...
local function w_init(m, arg)
  ...
  elseif m.__typename == 'cudnn.SpatialConvolutionMM' then
    m:reset(method(m.nInputPlane*m.kH*m.kW, m.nOutputPlane*m.kH*m.kW))
  elseif m.__typename == 'cudnn.LateralConvolution' then
    m:reset(method(m.nInputPlane*1*1, m.nOutputPlane*1*1))
  elseif m.__typename == 'cudnn.VerticalConvolution' then
    m:reset(method(1*m.kH*m.kW, 1*m.kH*m.kW))
  elseif m.__typename == 'cudnn.HorizontalConvolution' then
    m:reset(method(1*m.kH*m.kW, 1*m.kH*m.kW))
  elseif m.__typename == 'cudnn.Linear' then
    m:reset(method(m.weight:size(2), m.weight:size(1)))
  elseif m.__typename == 'nn.Linear' then
    m:reset(method(m.weight:size(2), m.weight:size(1)))
  elseif m.__typename == 'cudnn.TemporalConvolution' then
    m:reset(method(m.weight:size(2), m.weight:size(1)))
  elseif m.__typename~='inn.SpatialSameResponseNormalization' then
    return m
  end
  if m.bias then
    m.bias:zero()
  ...
  if(m.modules~=nil) then
    if(type(m.modules) == 'table') then
      for i=1,#m.modules do
        local child = m.modules[i]
        w_init(child,arg)
      end
    end
  end
